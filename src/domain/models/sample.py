"""
Domain model pro audio sample - čistá business logika bez závislostí
"""
...
class SampleMetadata:
    """
    Domain model pro audio sample s rozšířenou analýzou.
    Refaktorovaná verze - zachovává zpětnou kompatibilitu.
    """
    ...
    def mark_as_analyzed(self) -> None:
        """Označí sample jako analyzovaný."""
        self.analyzed = True
    ...
    def is_valid_for_mapping(self) -> bool:
        ...
        Returns:
            True pokud je sample analyzovaný, není filtrovaný a má detekovanou MIDI notu.
        """
        return (
            self.analyzed and
    ...
    def __str__(self):
        vel_str = f"{self.velocity_amplitude:.6f}" if self.velocity_amplitude else "None"
        return f"SampleMetadata({self.filename}, MIDI: {self.detected_midi}, " \
               f"Velocity amplitude: {vel_str}, Filtered: {self.is_filtered})"

    def __repr__(self):
        return self.__str__()

    def get_pitch_info(self) -> str:
        """Vrátí formátovanou informaci o pitch - LEGACY metoda pro kompatibilitu."""
        if not self.detected_midi:
            return "No pitch detected"

        # Inline MIDI to note conversion (temporary)
        notes = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
        octave = (self.detected_midi // 12) - 1
        note = notes[self.detected_midi % 12]
    ...
    def get_amplitude_info(self) -> str:
        """Vrátí formátovanou informaci o amplitude - LEGACY metoda pro kompatibilitu."""
